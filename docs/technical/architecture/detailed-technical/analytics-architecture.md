...
**Strategic Alignment**: PenguinMails' analytics architecture directly supports our market leadership goals by providing **enterprise-grade data intelligence** that delivers actionable insights for campaign optimization and business growth, establishing technical credibility in the competitive landscape.

**Technical Authority**: Our architecture integrates **PostgreSQL + PostHog** instead of the originally proposed Convex OLAP approach, providing the perfect balance of cost-effectiveness, developer experience, and real-time analytics capabilities with comprehensive queue-driven ETL pipeline.
...
**User Journey Integration**: This analytics foundation enables your complete data journey from [campaign performance monitoring](/docs/features/analytics) through [business intelligence insights](/docs/business/analytics/overview) to [advanced optimization recommendations](/docs/technical/architecture/detailed-technical/implementation-roadmap) - providing the data intelligence that powers your success.
...
CREATE TABLE daily_analytics (
    ...
    open_rate DECIMAL(5,2) GENERATED ALWAYS AS ()
        CASE WHEN emails_sent > 0
             THEN ROUND((emails_opened::DECIMAL ) * 100, 2)
             ELSE 0 END
    ) STORED,
    click_rate DECIMAL(5,2) GENERATED ALWAYS AS ()
        CASE WHEN emails_sent > 0
             THEN ROUND((emails_clicked::DECIMAL ) * 100, 2)
             ELSE 0 END
    ) STORED,
    bounce_rate DECIMAL(5,2) GENERATED ALWAYS AS ()
        CASE WHEN emails_sent > 0
             THEN ROUND((emails_bounced::DECIMAL ) * 100, 2)
             ELSE 0 END
    ) STORED,
    reply_rate DECIMAL(5,2) GENERATED ALWAYS AS ()
        CASE WHEN emails_sent > 0
             THEN ROUND((emails_replied::DECIMAL ) * 100, 2)
             ELSE 0 END
    ) STORED,
    created TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
...
- **Staging**: Managed Postgres ($15-25)
- **Production**: Auto-scaling Postgres ($25-100)
